...
bool HelloWorld::init()
{
    ...
    do 
    {
		...
		ballShape   = NULL;
			...
			// Корзина как цель для мяча - всего лишь промтранмтво между двумя линиями
...
void HelloWorld::Tick(float dt)
{
	...
	_world->Step(dt, velocityIterations, positionIterations);
		...
		// Перебираем все объектвы в мире и делаем анимацию движения тех,
		// к которым привязн какой-либо спрайт
...
{
		// Если mouseJoint уже есть, то нет смысла создавать еще один
	if ( ! mouseJoint )
	{
		...
		if (hit)
		{
			...
			this->addChild(crossMark, 1);
				...
				// Позиция мяча до отрыва. Для вычисления возвращающей силы.
...
void HelloWorld::ccTouchEnded(CCTouch* pTouch, CCEvent* event)
{
	...
	if (mouseJoint){				
		...
		// Действия, которые должны быть не здесь, но пусть пока побудут, как заготовка
